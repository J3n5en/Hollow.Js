 ...
 * @Last Modified time: 2017-05-25 23:24:21
...
module.exports = (app) => {
  ...
  return async (ctx, next) => {
    ...
    // console.log(path.join(app.settings.baseDir, ctx.path.replace(`/${options.prefix}`, '')))
    if (!fs.existsSync(path.join(app.settings.baseDir, ctx.path.replace(`/${options.prefix}`, `/${options.root}`)))) return;
